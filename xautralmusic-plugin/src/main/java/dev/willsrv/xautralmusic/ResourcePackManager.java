...
public class ResourcePackManager {
    ...
    public boolean generatePack() throws Exception {
        ...
        try (FileOutputStream fos = new FileOutputStream(pack);
            ...
            int minFormat = 34;
            int maxFormat = Math.max(packFormat, 88);
            String mcmeta = """
                    {
                      "pack": {
                        "pack_format": %d,
                    ...
                    """.formatted(packFormat, escapeJson(description), minFormat, maxFormat);
    ...
    private ConvertedAudio convertAudio(File input, File tmpDir) {
        ...
        // Spec Minecraft: Ogg Vorbis, 48000Hz max, mono=posicional sfx, stereo=music con stream:true, sin metadata extra
        List<String> cmd;
        if (channels != null) {
        ...
        try {
            ...
            if (exit == 0 && tmpOgg.exists() && tmpOgg.length() > 0) {
                ...
                List<String> cmd2;
                if (channels != null) {
                    cmd2 = List.of(ffmpeg, "-y", "-i", input.getAbsolutePath(), "-vn", "-map_metadata", "-1", "-c:a", "libvorbis", "-ac", channels, tmpOgg.getAbsolutePath());
                } else {
                    cmd2 = List.of(ffmpeg, "-y", "-i", input.getAbsolutePath(), "-vn", "-map_metadata", "-1", "-c:a", "libvorbis", tmpOgg.getAbsolutePath());
                }
