...
public class ResourcePackManager {
    ...
    private ConvertedAudio convertAudio(File input, File tmpDir) {
        ...
        // Detecta canales óptimos: sfx -> mono (posicional), music -> stereo (no posicional)
        // Si el archivo está en sfx/ fuerza mono 1, si es music/ stereo 2, sino respeta original
        ...
        List<String> cmd;
        if (channels != null) {
            cmd = List.of(ffmpeg, "-y", "-i", input.getAbsolutePath(), "-c:a", "libvorbis", "-q:a", String.valueOf(quality), "-ar", String.valueOf(rate), "-ac", channels, tmpOgg.getAbsolutePath());
        } else {
            cmd = List.of(ffmpeg, "-y", "-i", input.getAbsolutePath(), "-c:a", "libvorbis", "-q:a", String.valueOf(quality), "-ar", String.valueOf(rate), tmpOgg.getAbsolutePath());
        }
        try {
            ...
            if (exit == 0 && tmpOgg.exists() && tmpOgg.length() > 0) {
                ...
                // fallback: intenta con -c:a libvorbis sin quality (manteniendo canal si aplica)
                List<String> cmd2;
                if (channels != null) {
                    cmd2 = List.of(ffmpeg, "-y", "-i", input.getAbsolutePath(), "-c:a", "libvorbis", "-ac", channels, tmpOgg.getAbsolutePath());
                } else {
                    cmd2 = List.of(ffmpeg, "-y", "-i", input.getAbsolutePath(), "-c:a", "libvorbis", tmpOgg.getAbsolutePath());
                }
