...
public class ResourcePackManager {
    ...
    public boolean generatePack() throws Exception {
        ...
        try (FileOutputStream fos = new FileOutputStream(pack);
            ...
            // 1.21.9+ (26.x): use min_format/max_format; older: pack_format + supported_formats
            String mcmeta;
            if (packFormat >= 65) {
                // New format for 1.21.9+ / 26.x
                // min_format=34 covers 1.21+ (old enough for most ViaBackwards clients)
                mcmeta = """
                        {
                          "pack": {
                            "description": "%s",
                        ...
                        """.formatted(escapeJson(description), Math.min(packFormat, 34), Math.max(packFormat, 88));
            } else {
    ...
    private ConvertedAudio convertAudio(File input, File tmpDir) {
        ...
        File tmpWav = new File(tmpDir, base + "_pcm.wav");
        try {
