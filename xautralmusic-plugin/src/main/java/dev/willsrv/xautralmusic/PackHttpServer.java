...
public class PackHttpServer {
    ...
    public synchronized void start() {
        ...
        try {
            ...
            plugin.getSLF4JLogger().info("[XautralMusic] HTTP :{}/pack.zip -> {}", port, publicUrl);
            if (verbose) {
                String codespace = System.getenv("CODESPACE_NAME");
                if (codespace != null && !codespace.isBlank()) {
                    plugin.getSLF4JLogger().info("[XautralMusic] Codespace detectado: {}", publicUrl);
                } else if (host.equals("0.0.0.0")) {
                    plugin.getSLF4JLogger().info("[XautralMusic] Pack local en 0.0.0.0:{}", port);
                }
            }
        } catch (IOException e) {
    ...
    private class PackHandler implements HttpHandler {
        ...
        public void handle(HttpExchange exchange) throws IOException {
            ...
            boolean verbose = plugin.getConfig().getBoolean("verbose", false);
            if (verbose) plugin.getSLF4JLogger().info("[XautralMusic] HTTP {} {} desde {}", method, exchange.getRequestURI(), remote);
            ...
            if ("HEAD".equalsIgnoreCase(method)) {
                ...
                if (verbose) plugin.getSLF4JLogger().info("[XautralMusic] HEAD {} -> 200 ({} bytes)", exchange.getRequestURI(), data.length);
                return;
            }
            exchange.sendResponseHeaders(200, data.length);
            try (OutputStream os = exchange.getResponseBody()) { os.write(data); }
            if (verbose) plugin.getSLF4JLogger().info("[XautralMusic] GET {} -> 200 ({} bytes)", exchange.getRequestURI(), data.length);
        }
    }
}
