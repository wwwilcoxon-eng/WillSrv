...
public final class PhaxiCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!(sender instanceof Player player)) {
            sender.sendMessage(Tr.t(null, "Solo jugadores.", "Only players."));
            return true;
        }
        if (args.length == 0) {
            sendHelp(player);
        ...
        switch (sub) {
            case "set" -> {
                ...
                if (!name.matches("^[a-zA-Z0-9_\\-]{1,16}$")) {
                    player.sendMessage(Tr.t(player, "§6[Phaxi] §cNombre inválido. Usa 1-16 letras/números/_/-", "§6[Phaxi] §cInvalid name. Use 1-16 letters/numbers/_/-"));
                    return true;
                }
            ...
            case "send" -> {
                if (args.length < 3) {
                    player.sendMessage(Tr.t(player, "§6[Phaxi] §cUso: /phaxi send <jugador> <here | x y z [mundo]>", "§6[Phaxi] §cUsage: /phaxi send <player> <here | x y z [world]>"));
                    return true;
                }
            ...
            case "list" -> {
                ...
                if (homes.isEmpty()) {
                    player.sendMessage(Tr.t(player, "§6[Phaxi] §cNo tienes casas. Usa §e/phaxi set [nombre]", "§6[Phaxi] §cYou have no homes. Use §e/phaxi set [name]"));
                } else {
                    player.sendMessage(Tr.t(player, "§6[Phaxi] §aTus casas:", "§6[Phaxi] §aYour homes:"));
            ...
            case "del", "delete", "remove" -> {
                if (args.length < 2) {
                    player.sendMessage(Tr.t(player, "§6[Phaxi] §cUsa §e/phaxi del <nombre>", "§6[Phaxi] §cUse §e/phaxi del <name>"));
                    return true;
                }
                String name = args[1];
                if (manager.delHome(player, name)) {
                    player.sendMessage(Tr.t(player, "§6[Phaxi] §aCasa '" + name + "' eliminada.", "§6[Phaxi] §aHome '" + name + "' deleted."));
                } else {
                    player.sendMessage(Tr.t(player, "§6[Phaxi] §cNo tienes casa '" + name + "'.", "§6[Phaxi] §cYou don't have home '" + name + "'."));
                }
            }
    ...
    private void sendHelp(Player p) {
        boolean en = Tr.isEnglish(p);
        if (en) {
            p.sendMessage("§6[Phaxi] §7Commands:");
            p.sendMessage(" §e/phaxi set [name] §7- Save your current home (default: casa) and show coords");
            p.sendMessage(" §e/phaxi go [name] §7- Call Phaxi (1 XP per 30 blocks, confirm with click)");
            p.sendMessage(" §e/phaxi accept/deny §7- Accept/deny XP cost");
            p.sendMessage(" §e/phaxi send <player> <here|x y z [world]> §7- Send Phaxi to another (2 XP per 35 blocks)");
            p.sendMessage(" §e/phaxi list §7- List your homes");
            p.sendMessage(" §e/phaxi del <name> §7- Delete a home");
        } else {
